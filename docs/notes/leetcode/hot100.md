...
![](https://cansiny.oss-cn-shanghai.aliyuncs.com/images/1620836124658.png)

单看每个元素，都有两种选择：选入子集，或不选入子集,比如`[1,2,3]`，先看 1，选 1 或不选 1，都会再看 2，选 2 或不选 2，以此类推。

考察当前枚举的数，基于选它而继续，是一个递归分支；基于不选它而继续，又是一个分支。用索引`index`代表当前递归考察的数字`nums[index]`。
...
在 DFS 函数中，如果字符和 word 字符串对应，则继续比较相邻字符，否则返回`false`结束
...
`DIRECTIONS`数组是偏移量数组，代表位置的移动，是解二维数组的常用技巧，在本题中`DIRECTIONS`数组里面的子元素的顺序无关紧要
